...
class Calendar extends Component {
    ...
    this.setState(
      ...
      () => {
        if (!doNotTriggerListeners) {
          ...
          _.invoke(this.props, 'onVisibleMonthsChange', [
  ...
  _handleDayInteraction(date, interaction) {
    ...
    if (
      ...
      const shouldUpdateMonth =
        disableMonthChange === undefined || !disableMonthChange;
  ...
  renderDay(day, id) {
    ...
    const option = this.props.options?.find(
      (o) => o.time === dayjs(day).format('YYYY-MM-DD'),
    );
  ...
  renderHeader() {
    ...
    if (current) {
      ...
      if (
        displayLoadingIndicator &&
        !(markedDates && markedDates[lastMonthOfDay])
      ) {
        indicator = true;
      }
    }
